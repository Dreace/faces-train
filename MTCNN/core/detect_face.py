...
def detect_face(images, minsize, pnet, rnet, onet, threshold, factor, device):
    if isinstance(images, (np.ndarray, torch.Tensor)):
        if isinstance(images, np.ndarray):
            images = torch.as_tensor(images.copy(), device=device)
    ...
    images = images.permute(0, 3, 1, 2).type(model_dtype)
    ...
    offset = 0
    for scale in scales:
        im_data = imresample(images, (int(h * scale + 1), int(w * scale + 1)))
    ...
    if len(boxes) > 0:
        ...
        for k in range(len(y)):
            if ey[k] > (y[k] - 1) and ex[k] > (x[k] - 1):
                img_k = images[image_inds[k], :, (y[k] - 1):ey[k], (x[k] - 1):ex[k]].unsqueeze(0)
